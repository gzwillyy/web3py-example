...
# 输入交易多少Cats
cats_amount = console.log_input("卖出的CATS数量 : ")
cats_amount = int(cats_amount)

# 构建交易 用Cats买BNB
# 创建批准交易
approve_tx = cats_contract.functions.approve(pan_router_contract_address, cats_balance).buildTransaction({
    # str 发送方 地址或ens
    "from": sender_address,
    # int
...
# 签名授权批准交易
approve_signed = web3.eth.account.sign_transaction(approve_tx,private_key=config.PRIVATE_KEY)

# 发送授权交易
approve_tx_hash = web3.eth.send_raw_transaction(approve_signed.rawTransaction)
approve_trader_hash = web3.toHex(approve_tx_hash)
console.log_address("授权hash : ", approve_trader_hash)


# 1) 手动生成事务
pancake_tx = contract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(
    cats_amount, #卖出的token数量
    0,  # 最低获得的代币数量
    [token,spend],  # 路由路径 token bnb 
